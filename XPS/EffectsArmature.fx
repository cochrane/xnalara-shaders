...
/**************************
* Lesser bone matrices
*/
float4x4 BoneMatrices[52]; // changed from 59 to 52 to make space for normal map switch. If somebody need more space, then dont decremtent this lesser the 42, because this slow down the CPU.
/**************************
* New register for normal map switching
* value range from 0-5
* XNALara use tangent space normalmaps with RGB == X+Y-Z+
* 0 == X+
* 1 == X-
* 2 == Y+
* 3 == Y-
...
* 5 == Z-
*/
int TangentSpace_R = 0; // red channel stored as X+ axis
int TangentSpace_G = 3; // green channel stored as Y- axis
int TangentSpace_B = 4; // blue channel stored as Z+ axis
...
BumpVertexShaderOutput BumpVS(VertexShaderInput input)
{
    ...
    /******************
    * New part to swap the orientation depending of the user settings
    * Begin of CHANGE
    * RGB -- 0 3 4 -- X+Y-Z+
    */
    float3 v[6];
    v[0] = tangentU;
    v[1] = tangentU * -1;
    v[2] = tangentV * -1; // Not a BUG. XNALara use Y+ to change the usual direction
    v[3] = tangentV; // Not a BUG. XNALara use Y- as standard direction
    v[4] = normal;
    v[5] = normal * -1;
    
    output.TangentToWorld = mul(float3x3(v[TangentSpace_R], v[TangentSpace_G], v[TangentSpace_B]), fullTransform);
    /******************
    * End of new part
    * END of CHANGE
    * RGB -- 0 3 4 -- X+Y-Z+
